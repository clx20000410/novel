  ...
  <div class="p-8 bg-white rounded-2xl shadow-2xl fade-in">
    ...
    <div v-if="isGenerating" class="text-center py-12">
      ...
      <div class="space-y-4">
        ...
        <p v-if="isTakingLongerThanExpected" class="text-sm text-amber-700">
          生成耗时较长（已等待 {{ Math.floor(timeElapsed) }} 秒），请继续等待；如需重试，可先取消再重新生成。
        </p>
...
const maxTime = 180 // 进度条“预计耗时”（秒）
const overtimePromptSeconds = 30 * 60 // 超时提示阈值（秒），到点询问是否继续等待（30分钟）

let progressTimer: ReturnType<typeof setInterval> | null = null
let overtimePromptTimer: ReturnType<typeof setTimeout> | null = null
...
const isTakingLongerThanExpected = computed(() => timeElapsed.value >= maxTime)
...
const clearTimers = () => {
  ...
  if (overtimePromptTimer) {
    clearTimeout(overtimePromptTimer)
    overtimePromptTimer = null
  }
}
...
const generateBlueprint = async () => {
  ...
  // 超时提示：到点询问是否继续等待（不自动中止）
  overtimePromptTimer = setTimeout(async () => {
    if (!isGenerating.value || overtimePrompted) {
      return
    }
    overtimePrompted = true
    const shouldContinue = await globalAlert.showAlert(
      '生成蓝图已超过30分钟，可能是网络/服务繁忙导致。是否继续等待？\n\n- 继续等待：保持当前请求不变\n- 取消生成：中止请求并可稍后重试',
      'confirmation',
      '生成超时',
      { confirmText: '继续等待', cancelText: '取消生成', showCancel: true }
    )
    if (!shouldContinue) {
      cancelGeneration()
    }
  }, overtimePromptSeconds * 1000)

  try {
    // 直接调用store中的API
    console.log('开始调用generateBlueprint API...')
    const response = await novelStore.generateBlueprint({ signal: controller.signal })
    ...
    if (isAbortError) {
      globalAlert.showError('请求已被中止。请稍后重试。', '请求中止')
      return
    }
