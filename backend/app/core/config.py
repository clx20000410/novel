...
class Settings(BaseSettings):
    ...
    llm_stream_connect_timeout_seconds: float = Field(
        default=10.0,
        gt=0,
        env="LLM_STREAM_CONNECT_TIMEOUT_SECONDS",
        description="LLM 流式调用的连接超时秒数，用于识别服务不可达/网络问题",
    )
    blueprint_generation_timeout_seconds: float = Field(
        default=1800.0,
        gt=0,
    ...
    def _split_cors_list(cls, value):  # type: ignore[override]
        if value is None:
            return []
