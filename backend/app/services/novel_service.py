...
class NovelService:
    ...
    _INSPIRATION_ACTIVE_STATUSES: tuple[str, ...] = (
        "draft",
        "concept_in_progress",
        "concept_complete",
    )
    ...
    def build_project_summary(self, project: NovelProject) -> NovelProjectSummary:
    ...
    async def get_active_inspiration_project(self, user_id: int) -> Optional[NovelProject]:
        stmt = (
            select(NovelProject)
            .where(
                NovelProject.user_id == user_id,
                NovelProject.status.in_(self._INSPIRATION_ACTIVE_STATUSES),
            )
    ...
    async def abandon_active_inspiration_projects(self, user_id: int) -> int:
        now = datetime.now(timezone.utc)
        result = await self.session.execute(
            ...
            .values(status="concept_abandoned", updated_at=now)
        )
        await self.session.commit()
    ...
    async def list_projects_for_user(self, user_id: int) -> List[NovelProjectSummary]:
        ...
        for project in projects:
            summaries.append(self.build_project_summary(project))
        return summaries
    ...
    # 蓝图管理
    # ------------------------------------------------------------------
    async def get_blueprint(self, project_id: str) -> dict | None:
        """获取蓝图数据（字典形式），用于断点续传检查。"""
        ...
        relationships = [
            {
                "character_from": r.character_from,
                "character_to": r.character_to,
                "description": r.description,
            }
            for r in rels_result.scalars()
        ]
        ...
        chapter_outline = [
            ...
            for o in outlines_result.scalars()
        ]
        ...
        return {
            ...
            "full_synopsis": record.full_synopsis,
            "world_setting": record.world_setting,
    ...
    def _build_chapter_schema(
        ...
        *,
        ...
        if chapter:
            ...
            if word_count == 0 and chapter.selected_version and chapter.selected_version.content:
                word_count = len(chapter.selected_version.content or "")
