...
class AuthService:
    ...
    def __init__(self, session):
        ...
        self._code_ttl = _CODE_TTL_SECONDS
        self._send_interval = _SEND_INTERVAL_SECONDS
    ...
    async def send_verification_code(self, email: str) -> None:
        ...
        record = await self.session.get(EmailVerificationCode, email)
        ...
        record.expires_at = now + timedelta(seconds=self._code_ttl)
    ...
    def _hash_code(self, code: str) -> str:
        raw = f"{code}:{settings.secret_key}".encode("utf-8")
                    ...
                    <tr>
                        <td align=\"center\" style=\"background-color: #f9fafb; padding: 24px; border-top: 1px solid #e5e7eb;\">
                            ...
                            <p style=\"font-family: Arial, Helvetica, sans-serif; font-size: 12px; color: #9ca3af; margin: 8px 0 0;\">
                                &copy; {datetime.now(timezone.utc).year} 拯救小说家. All rights reserved.
                            </p>
                        </td>
                    </tr>
                </table>
